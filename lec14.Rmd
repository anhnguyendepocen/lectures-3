...
* Both are linear in $x$: LDA$\longrightarrow \alpha_0 + \alpha_1^\top x$, Logit$\longrightarrow \beta_0 + \beta_1^\top x$.

* But the parameters are estimated differently.

* Examine the joint distribution of $(X,y)$:
  - \alg{LDA} $\prod_i f(x_i,y_i) = \underbrace{\prod_i f(X_i | y_i)}_{\textrm{Gaussian}}
  \underbrace{\prod_i f(y_i)}_{\textrm{Bernoulli}}
  - \alo{Logistic} $\prod_i f(x_i,y_i) = \underbrace{\prod_i f(y_i | X_i)}_{\textrm{Logistic}}
  \underbrace{\prod_i f(X_i)}_{\textrm{Ignored}}
...
* If we have many columns in $X$ ($p$)
  ...
  - LDA estimates $2*p + p(p+1)/2 + 1$
  - QDA estimates $2*p + p(p+1) + 1$

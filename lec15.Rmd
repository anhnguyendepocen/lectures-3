...
4. Now, we can estimate the distribution of $\widehat{f}(Z_1,\ldots,Z_n)$ by looking at the $B$ draws, $\widehat{f}_b^*$. 
...
\]
This process is known as {\it Bagging} (bootstrap aggregation).
...
library(plyr)
library(kableExtra)
tab = ddply(long, ~predictor, fun)
tab$prediction = c(0,1,0,0,1,0,1)
kable(tab[,c(4,2,3)], 'latex') %>% group_rows('bagging', 1,2) %>% group_rows('null', 3,3) %>%
